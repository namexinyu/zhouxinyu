...
class PageSide extends React.PureComponent {
    ...
    handleClickItem(obj) {
        console.log(obj);
        setParams('state_sideNav', {selectedKeys: obj.key != '/ec/main/work-bench' ? [obj.key] : []});
        browserHistory.push({
            pathname: obj.key
        });
    }
        ...
        return (
            <Sider>
                <Menu defaultSelectedKeys={['1']}
                      ...
                      onOpenChange={(openKeys) => setParams('state_sideNav', {openKeys: openKeys})}
                      mode="inline"
                      theme="dark"
                      inlineCollapsed={false} onClick={this.handleClickItem} selectable={false}>
                    {navItems}
                </Menu>
            </Sider>
        );
...
//                     if (sItems.length) {
//                         navItems.push(
//                             <div className={'w-100 ' + (parentNav.id === item.id ? 'd-blocks' : 'd-none')}>
//                                 {sItems}
//                             </div>
//                         );
//                     }
