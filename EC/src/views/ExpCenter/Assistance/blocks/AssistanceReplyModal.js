...
class AssistanceReplyModal extends React.PureComponent {
    ...
    componentWillUpdate(nextProps, nextState) {
        ...
        if (closeFetch.status == 'success') {
            message.info("设置结案成功");
            this.hideModal();
        } else if (closeFetch.status == 'error') {
        ...
        if (gradeFetch.status == 'success') {
            message.info("评价委托成功");
            this.hideModal();
        } else if (gradeFetch.status == 'error') {
            let res = closeFetch.response;
            let text = '';
            if (res && res.Desc) text = ':' + res.Desc;
        ...
        if (replyFetch.status == 'success' && this.props.detail.ReplyDepartEntrustFetch.status != 'success') {
            ...
            message.info("回复成功");
            this.queryReplyList();
        } else if (replyFetch.status == 'error') {
            let res = closeFetch.response;
            let text = '';
            if (res && res.Desc) text = ':' + res.Desc;
        ...
        return (
            <Modal width={720}
                ...
                <div className="bg-white pl-16 pr-16">
                    <div>{`${this.AllDepartEnum[info.SourcePlatformCode]} - ${info.EntrustPerson} 提问：`}</div>
                            ...
                            <Row gutter={15}>
                                ...
                                <Col className="gutter-box" span={12}>
                                    <FormItem className={grade.Grade >= 3 ? '' : 'display-none'}>
                                        <Input placeholder="请填写不满意原因(不少于五个字)"
