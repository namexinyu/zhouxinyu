...
export default class AssistanceList extends React.PureComponent {
    constructor(props) {
        ...
        this.columns = [
            ...
            {title: '委托人', dataIndex: 'EntrustPerson'},
            {title: '委托问题', dataIndex: 'Content'},
            {title: '处理人', dataIndex: 'HandlePerson'},
            {
                title: '委托状态', key: 'EntrustStatus',
                render: (text, record) => {
                    return (<div className={record.EnableStatus == 2 ? 'color-primary' : 'color-danger'}>
        ...
        this.formItems = [
            ...
            {name: 'EntrustStatus', label: "委托状态", itemType: 'Select', type: 'enum', enum: this.eCloseStatus},
            {name: 'CloseStatus', label: "结案状态", itemType: 'Select', type: 'enum', enum: this.eIsNewMsgStatus},
    ...
    doQuery(data) {
        ...
        for (let key of ["EntrustStatus", "CloseStatus", "Grade", "SourceDepartID"]) {
            if (queryParams[key] && queryParams[key].value == '-9999') delete queryParams[key];
        }
        // 替换委托部门为委托平台
        if (queryParams.SourceDepartID) queryParams.SourcePlatformCode = {value: queryParams.SourceDepartID.value};
