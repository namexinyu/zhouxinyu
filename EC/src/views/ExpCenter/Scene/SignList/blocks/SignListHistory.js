...
import SceneService from "../../../../../services/ExpCenter/SceneService";
                                    ...
                                    <Col className="gutter-row" span={6}>
                                        <FormItem {...fLayout} label="企业">
                                            {recruitNameList.length > 0 ? (
                                                <AutoCompleteInput
                                                    // defaultValue={data.otherParams.RecruitName}
                                                    value={data.otherParams.CheckinRecruitName}
                                                    allowClear={true}
                                                    ...
                                                    handleChange={value => {
                                                        this.handleSetParam('CheckinRecruitName', value, 'otherParams');
                                                        if (queryParams.CheckinRecruitID) {
                                                            let target = recruitNameList.find((item) => item.RecruitID == queryParams.CheckinRecruitID);
                                                            if (target && target.RecruitName != value) {
                                                                this.handleSetParam('CheckinRecruitID', undefined);
                                                            }
                                                        }
                                                    }}
                                                    // handleSearch={value => console.log(value)}
                                                    handleSelect={(data, index) => {
                                                        this.handleSetParam('CheckinRecruitID', data.RecruitID);
                                                    }}
                                                />) : ''}
                                        </FormItem>
                                    </Col>
                ...
                render: (text, record) => {
                    ...
                    if (record.ChargeAmount) {
                        if (record.RefundAmount > 0) {
                            ...
                            chargeElm = <a onClick={() => this.handleShowRefundModal(record)}>退费</a>;
                        }
                    }
