...
function translatePostData(params, opts) {
    let token = AppSessionStorage.LOGIN_INFO_SESSION_STORAGE.getItem('Token') || AppSessionStorage.LOGIN_INFO_SESSION_STORAGE.getItem('token') || '';
    if (!token) {
        goToLogin();
        return false;
    }
    ...
    let UID = JSON.parse(sessionStorage.getItem("mams_session_login_info")).employeeId;
    return {
