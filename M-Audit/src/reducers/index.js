  ...
  if (reducers) {
    Object
      ...
      .forEach(actionType => {
        ...
        if (!actionReducers[actionType]) 
          actionReducers[actionType] = [];
...
function setInitialState(ctx, paths) {
  ...
  for (let i = 0; i < lastIndex; i++) 
    node = node[paths[i]];
  if (!node[paths[lastIndex]]) 
    node[paths[lastIndex]] = initialStates[paths.join('.')] || {};
}
