...
class RecruitmentInfo extends React.PureComponent {
    ...
    componentWillMount() {
        ...
        if (location.lastKey !== location.key && !(location.state && location.state.clickTab)) {
            ...
            ActionMAMSRecruitment.GetTagStockList({
                TagName: '',
                RecordIndex: 0,
                RecordSize: 9999
            });
    ...
    componentWillReceiveProps(nextProps) {
        ...
        // if (nextProps.list.GetTagStockListFetch.status === 'success' || nextProps.list.GetTagStockListFetch.status === 'error') {
        //     setFetchStatus('state_mams_recruitmentList', 'GetTagStockListFetch', 'close');
        //     setTimeout(() => {
        //         this.setState({
        //             isRefreshing: false
        //         });    
    ...
    doQuery(data) {
        ...
        param.CommonTagList = this.props.list.checkedTags.map((item) => {
            return {
    ...
    handleViewDetail = (record) => {
        console.log(record);
        browserHistory.push({
            pathname: '/broker/recruit/detail/' + record.RecruitTmpID,
            query: {
        ...
        return (
            <div className="recruitment-info-view">
                <div className="ivy-page-title">
                    <div style={{position: 'relative'}} className="flex flex--between flex--y-center">
                        <h1>招工资讯</h1>
                        {/* <span className={classnames('refresh-icon cursor-pointer', {
                          'refresh-active': this.state.isRefreshing 
                ...
                <div className="container-fluid pb-24">
                    <div style={{padding: 0, shadow: 'none', backgroundColor: 'inherit'}}>
                        <Row type="flex" justify="space-between" className="mb-16"
                             style={{padding: '15px', backgroundColor: '#ffffff', fontSize: 18}}>
                            <RecruitmentForm state_name={data.state_name} spread={data.spread}
                                             Phone={data.Phone}
                                             queryParams={data.queryParams}
                                             tagStockList={data.tagStockList}
                                             checkedTags={data.checkedTags}
                                             spreadMatchEnt={data.spreadMatchEnt}
                                             spreadCheckTag={data.spreadCheckTag}
                                             spreadTagType={data.spreadTagType}
                                             spreadMemberType={data.spreadMemberType}
                                             checkedTagStockList={data.checkedTagStockList}
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={7}>
                                <Affix offsetTop={34} target={() => document.querySelector('.ant-layout-content .container-fluid')}>
    ...
    requireInfo() {
        ...
        return (
            <div className="pt-8 pb-8 pl-16 pr-16"
                 style={{fontSize: '14px', lineHeight: '18px', minHeight: '360px'}}>
                ...
                render: (text, item) => {
                    return (
                        <div>
                            ...
                            <a onClick={this.handleEntrustRecruit.bind(this, item)} className="ml-10">纠错</a>
                        </div>
