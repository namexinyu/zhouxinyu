...
class LaborTestModal extends React.PureComponent {
  ...
  getExamDetail (params) {
    this.initData();
    this.setState({ RecordListLoading: true, CurrentID: this.state.CurrentID + 1 });
    return HttpRequest.post({
      url: env.api_url + '/BK_Exam/BK_User_GetQuestionTest',
      params: params
    }, {
        successDo: (res) => {
          if (res.Code == 0) {
        ...
        errorDo: (res) => {
          ...
          let td = {
                      ...
                      CurrentTestID: this.state.CurrentID,
                      RealName: 'data.RealName',
                      Year: '',
                      Jiguan: '',
                      Hunfou: '',
        ...
        <span>
          <Table
            ...
            columns={[{
              ...
              render: (text, record) => (
                <span>
                  <InputNumber disabled={this.state.disableInput} value={this.state.Birth} style={{ width: 80 }} min={1900} max={2050} onChange={this.saveYear} className="input-number--normal" />
              ...
              title: '操作',
              dataIndex: 'opration',
              render: (text, record) => <Button className="ml-8" disabled={this.state.disableInput} onClick={() => this.submitExam(record)}>提交</Button>
            }
          ...
          <Form style={{ display: (this.state.bShowNext ? '' : 'none') }}>
            ...
            <Row>
              <Col className="mb-24" span={24} style={{ textAlign: 'right' }}>
                ...
                <Button className="ml-8" onClick={this.handleEnd}>结束</Button>
              </Col>
            </Row>
          </Form>
        </span>
