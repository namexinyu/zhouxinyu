...
class BlockContact extends React.PureComponent {
  componentWillReceiveProps (nextProps) {
    ...
    const RecruitTmpID = nextProps.pocketInfo.lastestEnrollRecord.RecruitTmpID;
    const PocketRecruitId = nextProps.pocketInfo.blockId.value && nextProps.pocketInfo.blockId.value;
    ...
    if (nextProps.patchProcessDemandsFetch.status === 'success') {
      ...
      if (CaseLevel.value == "" && BirthYear.value == "" && Province.value == "" && Education.value == "" && MaritalStatus.value == "" && Wycas.value.length == 0 && WorkYearStr.value == "") {
        const getBlockContactModal = this.props.getBlockContactModal;

        Modal.confirm({
          className: "ModalConfirmModalConfirmModalConfirmModalConfirmModalConfirmModalConfirmModalConfirmModalConfirmModalConfirm",
          okText: "是",
          cancelText: "否",
          maskClosable: true,
          content: '是否是有效联系记录？',
          ...
          onCancel () {
            if (nextProps.processInfo.type == 'new') {
              getPersonPostInfo({
                user_id: userId,
                recruit_tmp_id: Number(PocketRecruitId),
                call_type: 2
              });
            } else {
              getPersonPostInfo({
      ...
      if (nextProps.personInfo.getPersonPostInfoFetch.status === 'success') {
            ...
            <Col span={24}>
              <div className="flex flex--y-center">
                <FormItem label="推荐人" className="flex__item form-item__zeromb flexible-form-item">
                  <span>{this.props.userInfo.InviteName || '-'}</span>
