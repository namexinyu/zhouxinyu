...
import BlockPersonPostModal from './BlockPersonpostModal';
import BlockPersonPostMessge from './BlockPersonPostMessge';
...
class MemberDetailBox extends React.PureComponent {
  ...
  componentWillMount () {
    ...
    getLatestEnrollRecord({
      BrokerID: brokerId,
      UserID: +this.props.router.params.memberId,
      CaseStatus: 1
    });

    // getMemberApplyInfo({UserID: +this.props.router.params.memberId});
  ...
  componentWillReceiveProps (nextProps) {
    ...
    const { pocketInfo, detailOperate, alarmClock, recommendList, processInfo, personInfo } = nextProps;
    // if (processInfo.updatePreSignFetch.status === 'success' || processInfo.insertPreSignFetch.status === 'success') {
    //   const { expectedSignRecruit } = this.props.pocketInfo;
    //   // 修改预签到成功，展示人岗信息
    //   getPersonPostInfo({
    //     call_type: 1,
    //     user_id: parseInt(this.props.router.params.memberId, 10),
    //     recruit_tmp_id: Number(expectedSignRecruit && expectedSignRecruit.value && +expectedSignRecruit.value.value)
    //   });
    // }

    // 口袋设置成功，展示人岗信息
    if (personInfo.is_ok === 1) {
      this.setState({
        personModal: true
      });
    } else if (personInfo.is_ok === 2) {
      this.setState({
        messgeModal: true
      });
    }
  ...
  handleShowRecommendModal () {
    if (!this.props.recommendList.createRecommendUserID || (this.props.recommendList.createRecommendUserID.value != this.props.router.params.memberId)) {
      setParams('state_broker_member_detail_recommend_list', { 'createRecommendUserID': { value: this.props.router.params.memberId } });
    }
    this.setState({
      showRecommend: true
    });
    ...
    this.props.form.validateFieldsAndScroll(['createRecommendName', 'createRecommendPhone'], (errors, values) => {
      if (!errors) {
        ...
        helpMemberRecommend({
          ...
          UserID: u_u && u_u.value ? u_u.value - 0 : parseInt(this.props.router.params.memberId, 10)
        });
      }
    });
  }
  ...
  handleSetPocket () {
    setParams('state_broker_member_detail_process', {
      editPocketUserID: { value: this.props.router.params.memberId },
  ...
  handleShowFactoryCheckin = () => {
    const param = {
      UserID: { value: this.props.router.params.memberId - 0 },
  ...
  handleShowSetSign = () => {
    ...
    if (PreSignInfo) {
      ...
      setParams(STATE_NAME_POCKET, {
        ...
        expectedSignUserId: {
          value: this.props.router.params.memberId + ''
        },
        expectedSignPlace: {
          value: existedHub ? {
  ...
  handleModalSetPickOk = () => {
    this.props.form.validateFieldsAndScroll(['expectedSignRecruit', 'expectedSignDate', 'expectedSignUserId', 'expectedSignPlace'], (errors, values) => {
      if (!errors) {
        ...
        if ((expectedSignDate && expectedSignDate.value) < moment()) {
          message.error('预签到日期不可以小于当前日期');
        } else {
          if (PreSignInfo) {
  ...
  getBlockContactModal = (type) => {
    this.setState({
      BlockContactModal: type
    });
    console.log(this.state.BlockContactModal);
  }
  ...
  render () {
    ...
    const userId = +this.props.router.params.memberId;
    let abnormalReason = '';
    ...
    return (
      <div className="member-detail-box">
          ...
          <Col span={24}>
            <div className="flex flex--y-center flex--between">
              <div className="flex__item flex flex--y-center">
                ...
                <BlockDetailInfoNewHeader {...this.props.detailInfo} />
            ...
            <div style={{ height: '100%', padding: 10 }} className="flex">
              <div>
                <Row>
                  <Col span={24}>
                    <div>
                      <BlockDetailInfoNew {...this.props.detailInfo} processInfo={this.props.processInfo} />
                    </div>
                  </Col>
                </Row>
                <Row className="mt-14">
                ...
                <BlockContact {...this.props.processInfo}
                  ...
                  getBlockContactModal={this.getBlockContactModal}
                  impressionInfo={this.props.impressionInfo}
                  expectationInfo={this.props.expectationInfo}
                    ...
                    <Row>
                      {Object.keys(this.props.pocketInfo.lastestEnrollRecord).length ? (
                        <BlockPocket allRecruitList={this.props.allRecruitList}
                          ...
                          getBlockContactModal={this.getBlockContactModal}
                          // getPersonPostInfo={this.getPersonPostInfo}
                          impressionInfo={this.props.detailInfo.impressionInfo}
                          router={this.props.router}
        ...
        <BlockContactModal
          pocketInfo={this.props.pocketInfo}
          memberId={parseInt(this.props.router.params.memberId, 10)}
          getBlockContactModal={this.getBlockContactModal}
          brokerId={brokerId}
          BlockContactModal={this.state.BlockContactModal}
          patchProcessDemandsFetch={this.props.detailInfo.patchProcessDemandsFetch}
          impressionInfo={this.props.detailInfo.impressionInfo}
          impressionConfigList={this.props.detailInfo.impressionConfigList} />
        <BlockPersonPostModal
          personInfo={this.props.personInfo}
          personModal={this.state.personModal}
          personPostModalCancel={this.personPostModalCancel}
        />
        <BlockPersonPostMessge
          messgeModal={this.state.messgeModal}
