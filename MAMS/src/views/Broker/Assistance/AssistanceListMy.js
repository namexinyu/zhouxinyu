...
export default class AssistanceListMy extends React.PureComponent {
    ...
    doQuery(data) {
        ...
        for (let key of ["EntrustStatus", "CloseStatus", "Grade", "TargetDepartID"]) {
            if (queryParams[key] && queryParams[key].value == '-9999') delete queryParams[key];
        }
        // 替换指派部门为指派平台
        if (queryParams.TargetDepartID) queryParams.TargetPlatformCode = {value: queryParams.TargetDepartID.value};
        ...
        GetDepartEntrustListMy({
            ...
            EntrustEmployeeID: AppSessionStorage.LOGIN_INFO_SESSION_STORAGE.getItem('employeeId'),
            ListType: 2, // 处理委托
