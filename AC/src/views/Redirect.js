...
class Redirect extends Component {
    componentWillMount() {
        ...
        if (token && accountList && accountList.length && employeeId) {
            ...
            setParams('state_ac_header_accountInfo', {
            ...
            if (r) {
                ...
                if (roleCode && roleCode.length > 0) {
                    ...
                    if (roleCode.indexOf(role.BOSS) !== -1 || roleCode.indexOf(role.MANAGER) !== -1 || roleCode.indexOf(role.ASSISTANT) !== -1) {
                        browserHistory.push({
                            pathname: '/ac/board'
                        });
                    } else if (roleCode.indexOf(role.MANAGER_ASSIST) !== -1) {
                        browserHistory.push({
                            pathname: '/ac/report/interview'
                        });
                    }
                }
            }
