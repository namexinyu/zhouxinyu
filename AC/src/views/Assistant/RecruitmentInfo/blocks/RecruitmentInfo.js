...
const thead = [
    {value: '', autoWidth: true, type: 'text'},
    {value: '企业', autoWidth: true, type: 'text', bold: true},
    {value: '工资', autoWidth: true, type: 'text', bold: true},
    {value: '年龄', autoWidth: true, type: 'text', bold: true},
    {value: '集合时间', autoWidth: true, type: 'text', bold: true},
    {value: '补贴', autoWidth: true, type: 'text', bold: true}
];
...
class RecruitmentInfo extends React.PureComponent {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.list.currentPage !== this.props.list.currentPage
            || nextProps.list.pageSize !== this.props.list.pageSize) {
    ...
    doQuery(data) {
        ...
        ActionMAMSRecruitment.GetMAMSRecruitmentList({...param, ...this.obtainQueryParam(data)});
    }

    obtainQueryParam(data) {
        let param = {
            RecordIndex: data.pageSize * (data.currentPage - 1),
            RecordSize: data.pageSize
        };
    ...
    handleRowDoubleClick = (record) => {
        if (!record.RecruitTmpID) return;
        
        const url = 'http://www.wodedagong.com/Enterprise/GeneralWorkerDetails/' + record.RecruitTmpID + '.html';
    ...
    getAge = (item) => {
        ...
        if (mf != '不限') {
            if (result) result += '\n';
            if (mf.includes(':')) result += '男女比例: ';
            result += `${mf}`;
        }
        return result;
    };

    getSubsidy = (SubsidyList) => {
        let list = SubsidyList || [];
        return list.reduce((pre, cur, index) => {
            if (index !== 0) pre += '\n';
            pre += `${this.eGenderAll[cur.Gender] ? this.eGenderAll[cur.Gender] : ''} ${cur.Day}天 ${(cur.Amount / 100).FormatMoney({fixed: 2})}元`;
            return pre;
        }, '');
    };

    handleExportExcel = () => {
        let params = this.obtainQueryParam(this.props.list);
        ...
        HttpRequest.post({
            ...
            params: {...params, RecordIndex: 0, RecordSize: 99999999}
        }, {
            successDo: (res) => {
                if (res.Data && res.Data.RecruitList)
                    this.listToExcel(res.Data.RecruitList);
            },
            errorDo: (res) => res
        }, {EmployeeMapID: true});
    };
    ...
    listToExcel(list) {
        let dataList = list.reduce((pre, cur) => {
            if (cur.PayType) { // 周薪薪
                pre[0].list.push(cur);
            } else if (cur.AreaCode == '320509') { // 吴江
                pre[1].list.push(cur);
            } else if (cur.AreaCode == '320583') { // 昆山
                pre[2].list.push(cur);
            } else if (cur.AreaCode == '320500') { // 苏州
                pre[3].list.push(cur);
            } else {
                pre[4].list.push(cur);
            }
        ...
        for (let dataItem of dataList) {
            dataItem.list.map((item, index) => {
                ...
                if (index === 0) row.push({
                    ...
                    rowSpan: dataItem.list.length,
                    autoWidth: true,
                    type: 'text'
                }); // 区域
                row.push({value: item.PositionName, autoWidth: true, type: 'text'}); // 企业
                row.push({value: item.SalaryInfo, autoWidth: true, type: 'text'}); // 工资
                row.push({value: this.getAge(item), autoWidth: true, type: 'text'}); // 年龄
                row.push({value: item.Time, autoWidth: true, type: 'text'}); // 集合时间
                row.push({value: this.getSubsidy(item.SubsidyList), autoWidth: true, type: 'text'}); // 补贴
                data.push(row);
            });
        }
    ...
    handleViewDetail = (record) => {
        browserHistory.push({
            pathname: '/ac/daily/recruit/detail/' + record.RecruitTmpID,
            query: {
                    ...
                    <Card bordered={false}>
                        <Row type="flex" justify="space-between" style={{fontSize: 18}}>
                            <RecruitmentForm state_name={data.state_name} spread={data.spread}
                                             ...
                                             pageState={data}
                                             obtainQueryParam={this.obtainQueryParam}
                ...
                <div className="container-fluid pb-24">
                    <Row gutter={16}>
                        <Col span={17}>
                            ...
                            {/* {this.IsManager && <Button onClick={this.handleExportExcel} size='small'>导出</Button>} */}
                        </Col>
                        <Col span={7}>
                            <Affix offsetTop={32}
                                   target={() => document.querySelector('.ant-layout-content .ivy-page')}>
