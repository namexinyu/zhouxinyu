...
class PickUpReport extends React.PureComponent {
    constructor(props) {
        super(props);
    ...
    doQuery(d) {
        ...
        ReportAction.RptGetPickUpList(param);
    }
                        ...
                        <div className="alert-container">
                            <div className="alert-div">
                                ...
                                <div className="line-2 color-success">
                                    {data.PreCheckinAddrTotal != null && data.PreCheckinAddrTotal != undefined ? data.PreCheckinAddrTotal : '--'}</div>
                                <div className="line-3">明日预接站总数</div>
                            </div>
                            <div className="alert-div">
                            ...
                            <div className="alert-div">
                                ...
                                <div className="line-2 color-success">
                                    {data.BrokerTotal ? (data.PreCheckinAddrTotal / data.BrokerTotal).FormatMoney({fixed: 2}) : '--'}</div>
                                <div className="line-3">平均明日预接站数</div>
                            </div>
                        </div>
    ...
    tableData(list = []) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            for (let n = 0; n < len; n++) {
                ...
                d_d_rx[i * 4 + 3] = g_g_r.PreCheckinAddrCount !== 0 ? g_g_r.PreCheckinAddrCount : f_e_e(g_g_r.PreCheckinAddrCount, 'td-div rbg-red');
            }
        }
        d_d_t = d_d_t.map((a_a) => {
    ...
    tableColumns(list = []) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            d_d_r1[i * 4] = '总计';
            d_d_r1[i * 4 + 1] = f_e_e(list[i].GroupPreCheckinAddrTotal, 'rc-orange');
            d_d_r1[i * 4 + 2] = '人均';
            d_d_r1[i * 4 + 3] = f_e_e(list[i].GroupBrokerTotal ? (list[i].GroupPreCheckinAddrTotal / list[i].GroupBrokerTotal).FormatMoney({fixed: 2}) : 0, 'rc-orange');
            d_d_r2[i * 4] = '段位';
            d_d_r2[i * 4 + 1] = '工号';
            d_d_r2[i * 4 + 2] = '经纪人';
            d_d_r2[i * 4 + 3] = '预接站';
