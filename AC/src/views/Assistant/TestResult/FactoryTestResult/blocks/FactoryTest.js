...
const { getFactoryTestList, GetBrokerDepartList } = TestResult;
...
class FactoryTest extends React.PureComponent {
  constructor(props) {
    super(props);
    const { DGList } = getAuthority();
    this.state = {
      page: (this.props.FacTestListInfo.pageParams.RecordIndex / this.props.FacTestListInfo.pageParams.RecordSize) + 1,
      pageSize: 40,
      RecordIndex: 0,
      querdata: {},
      DGList,
  ...
  getResultList = (pageParams = this.props.FacTestListInfo.pageParams) => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const pageQueryParams = this.props.FacTestListInfo.pageQueryParams;
        const queryParams = {
          ...pageQueryParams,
        ...
        getFactoryTestList({ ...queryParams, ...pageParams });
      }
    });
  }
  ...
  componentWillReceiveProps (nextProps) {
    // 翻页
    if (this.props.FacTestListInfo.pageParams !== nextProps.FacTestListInfo.pageParams) {
      this.getResultList(nextProps.FacTestListInfo.pageParams);
    }
  }
  ...
  render () {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, recoder) => {
          return (
            <span style={recoder.TodayCount >= 15 ? { color: '#00FF00' } : { color: '#FF0000' }}>{text}</span>
          );
        }
      },
      {
      ...
      },
      {
        ...
        render: (record) => {
          return (
            <Link to={{ pathname: `/ac/result/factory/detail/${record.BrokerID}`, query: { QueryDate } }}>查看</Link>
          );
        }
      }
    ];
...
export default Form.create()(FactoryTest);
