...
class CompanyEdit extends React.PureComponent {
    ...
    componentWillMount() {
        if (this.props.location.lastKey !== this.props.location.key && !(this.props.location.state && this.props.location.state.clickTab)) {
            ...
            if (this.props.routeParams.companyId === 'new') {
                resetState('state_servicer_labor_company_edit');
            } else {
                resetState('state_servicer_labor_company_edit');
                let checkId = QueryParam.getQueryParam(window.location.href, 'checkId');
                if (checkId) {
                    getCheckCompanyDetail({
                        LaborAuditID: parseInt(checkId, 10)
                    });
                } else {
                    getCompanyDetail({
                        LaborID: parseInt(this.props.routeParams.companyId, 10)
                    });
                }
            }
        }
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.editCompanyInfoFetch.status === 'success') {
            setFetchStatus('state_servicer_labor_company_edit', 'editCompanyInfoFetch', 'close');
        ...
        if (nextProps.editCompanyInfoFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_company_edit', 'editCompanyInfoFetch', 'close');
            let res = nextProps.editCompanyInfoFetch.response;
            Modal.error({
                title: window.errorTitle.normal,
                content: res ? (res.customDesc || res.Desc) : ''
            });
        }
        if (nextProps.editCheckCompanyInfoFetch.status === 'success') {
            setFetchStatus('state_servicer_labor_company_edit', 'editCheckCompanyInfoFetch', 'close');
            message.success('修改劳务公司信息成功');
            doTabPage({
                id: this.props.location.pathname + this.props.location.search
            }, 'close');
        }
        if (nextProps.editCheckCompanyInfoFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_company_edit', 'editCheckCompanyInfoFetch', 'close');
            Modal.error({
                title: window.errorTitle.normal,
                content: nextProps.editCheckCompanyInfoFetch.response.Desc
            });
        }
        if (nextProps.getCompanyDetailFetch.status === 'success') {
            setFetchStatus('state_servicer_labor_company_edit', 'getCompanyDetailFetch', 'close');
            let data = nextProps.getCompanyDetailFetch.response.Data;
            this.handleTranslateData(data);
            getCompanyAccountInfo({
                AccountID: data.AccountID
            });
        }
        if (nextProps.getCompanyDetailFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_company_edit', 'getCompanyDetailFetch', 'close');
            message.error(nextProps.getCompanyDetailFetch.response.Desc);
        }
        if (nextProps.getCheckCompanyDetailFetch.status === 'success') {
            setFetchStatus('state_servicer_labor_company_edit', 'getCheckCompanyDetailFetch', 'close');
            let data = nextProps.getCheckCompanyDetailFetch.response.Data;
            this.handleTranslateData(data);
            if (data.AccountID) {
                getCompanyAccountInfo({
                    AccountID: data.AccountID
        ...
        if (nextProps.getCheckCompanyDetailFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_company_edit', 'getCheckCompanyDetailFetch', 'close');
            message.error(nextProps.getCheckCompanyDetailFetch.response.Desc);
        }
        if (nextProps.getCompanyAccountInfoFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_company_edit', 'getCompanyAccountInfoFetch', 'close');
            message.error(nextProps.getCompanyAccountInfoFetch.response.Desc);
        }
        if (nextProps.getLaborBossContactInfoFetch.status === 'success') {
            setFetchStatus('state_servicer_labor_company_edit', 'getLaborBossContactInfoFetch', 'close');
            let data = nextProps.getLaborBossContactInfoFetch.response.Data;
            setParams('state_servicer_labor_company_edit', {
                contactName: {
                    value: data.BossName
                },
                contactMobile: {
...
export default Form.create({
    ...
    onFieldsChange: (props, fields) => {
        setParams('state_servicer_labor_company_edit', fields);
    }
