...
class OwnerEdit extends React.PureComponent {
    componentWillMount() {
        if (this.props.location.lastKey !== this.props.location.key && !(this.props.location.state && this.props.location.state.clickTab)) {
            if (this.props.routeParams.ownerId === '0' && !QueryParam.getQueryParam(window.location.href, 'checkId')) {
                resetState('state_servicer_labor_boss_edit');
            } else {
                resetState('state_servicer_labor_boss_edit');
                if (!QueryParam.getQueryParam(window.location.href, 'checkId')) {
                    getLaborBossCompanyList({
                        LBossID: parseInt(this.props.routeParams.ownerId, 10)
                    });
                }

                setParams('state_servicer_labor_boss_edit', {
                    laborBossName: {
                        value: decodeURIComponent(QueryParam.getQueryParam(window.location.href, 'laborBossName')) || ''
                    },
                    laborBossMobile: {
                        value: QueryParam.getQueryParam(window.location.href, 'laborBossMobile') || ''
                    },
                    cooperStatus: {
                        value: parseInt(QueryParam.getQueryParam(window.location.href, 'cooperStatus'), 10) === 1 ? true : false
                    },
                    laborBossIdCard: {
                        value: QueryParam.getQueryParam(window.location.href, 'laborBossIdCard') || ''
                    }
                });
                getClient(uploadRule.laborBossCertPicture).then((client) => {
                    let laborBossIdCardPositive = decodeURIComponent(QueryParam.getQueryParam(window.location.href, 'laborBossIdCardPositive'));
                    if (laborBossIdCardPositive) {
                        setParams('state_servicer_labor_boss_edit', {
                    ...
                    let laborBossIdCardOpposite = decodeURIComponent(QueryParam.getQueryParam(window.location.href, 'laborBossIdCardOpposite'));
                    if (laborBossIdCardOpposite) {
                        setParams('state_servicer_labor_boss_edit', {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.getLaborBossCompanyListFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_boss_edit', 'getLaborBossCompanyListFetch', 'close');
            Modal.error({
                title: window.errorTitle.normal,
                content: nextProps.getLaborBossCompanyListFetch.response.Desc
            });
        }
        if (nextProps.editLaborBossInfoFetch.status === 'success') {
            setFetchStatus('state_servicer_labor_boss_edit', 'editLaborBossInfoFetch', 'close');
        ...
        if (nextProps.editLaborBossInfoFetch.status === 'error') {
            setFetchStatus('state_servicer_labor_boss_edit', 'editLaborBossInfoFetch', 'close');
            Modal.error({
                title: window.errorTitle.normal,
                content: nextProps.editLaborBossInfoFetch.response.Desc
            });
        }
    }
...
export default Form.create({
    ...
    onFieldsChange: (props, fields) => {
        setParams('state_servicer_labor_boss_edit', fields);
    }
