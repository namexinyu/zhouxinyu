...
class TagModal extends React.PureComponent {
  ...
  handleOnOK = () => {
    this.props.form.validateFields((err, values) => {
      ...
      getMoveEvent({
        ...
        EventIDs: this.props.transformItems.map(item => item.EventID),
        ...
        if (res.Code === 0) {
          ...
          message.error(res.Data.Desc || '批量转移失败');
        }
      }).catch((err) => {
  ...
  render() {
    ...
    return (
      ...
      >
        <Form className="">
          <FormItem className="tag-modal-form__name" labelCol={{span: 6}} wrapperCol={{span: 18}} label="处理人变更">
            {getFieldDecorator('Processor', {
              ...
              >
                {
                  availableProcessors.map((item, i) => {
                    return (
          ...
          <FormItem labelCol={{span: 6}} wrapperCol={{span: 18}} label="备注">
            {getFieldDecorator('Remark', {
              ...
              <Input.TextArea autosize={{minRows: 4, maxRows: 8}} maxLength="100" />
            )}
          </FormItem>
        </Form>
      </Modal>
