...
import { Modal, Button, Row, Form, Input, Label, message } from 'antd';
import {UTIL} from 'mams-com';
const {LazyLoadMap} = UTIL;
                    ...
                    {<Row>
                        <Form layout="inline">
                            {this.props.enableEdit && <FormItem>
                                <Input type="text" style={{ width: '300px' }} placeholder="请输入企业位置" value={this.state.searchKey} onChange={(e) => this.handleSearchKeyChange(e)} />
                            </FormItem>}
                            {this.props.enableEdit && <FormItem>
                                <Button htmlType="button" type="primary" onClick={() => this.handleDoSearch()}>搜索</Button>
                            </FormItem>}
                            <FormItem label={'打卡范围半径（米）'}>
                                <Input type="number" readOnly={true} value={this.state.clockRadius} placeholder="打卡范围半径" />
                            </FormItem>
                            <FormItem label={'企业定位'}>
                                <Input type="text" readOnly={true} value={this.state.longlat} placeholder="经纬度" />
                            </FormItem>
                        </Form>
                        {this.props.enableEdit && <p className="mt-8 color-danger">温馨提示：1、输入关键词时，请尽量带上区域名称（如：昆山世硕）；2、【手势】【圆圈】工具可在地图拖动和画圆圈功能之间进行切换；3、打卡范围半径请通过<b>地图右上角</b>【圆圈】工具进行划定。鼠标点击处为圆圈范围中心点。</p>}
                    </Row>}
                    <Row className="mt-24">
                        <div style={{ width: ((this.props.enableEdit && this.state.searchResult.length) ? '80%' : '100%'), height: '600px', float: 'left' }} id="baiduMap"></div>
                        <div style={{ width: '20%', height: '600px', float: 'left', overflowX: 'hidden', overflowY: 'auto' }} id="mapResult" style={{ display: (this.props.enableEdit && this.state.searchResult.length) ? 'block' : 'none' }}></div>
                    </Row>
                </Modal>
            </div>
        );
